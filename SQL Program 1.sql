...
SELECT  SS.SEC, S.GENDER, COUNT(S.GENDER) AS COUNT
FROM STUDENT S, SEMSEC SS, CLASS C
WHERE S.USN = C.USN AND SS.SSID = C.SSID
GROUP BY SS.SEC , S.GENDER;

-- QUERY 3
## Create a view of Test1 marks of student USN ‘1DC20AI001’ in all subjects.
...
END IF;
SET C_AVG=C_SUM/2;
UPDATE IAMARKS SET FINALIA =C_AVG WHERE USN=C_USN AND SUBCODE=C_SUBCODE AND SSID=C_SSID;
CLOSE C_IAMARKS;
END;
//
CALL AVG_MARKS();
...
## If Final IA = 12 to 16 then CAT = ‘Average’ 
## If Final IA < 12 then CAT = ‘Weak’ 
SELECT S.USN, S.SNAME, S.ADDRESS, S.PHONE, S.GENDER, SUB.TITLE,
    (CASE
